...
function _isValidUrl(url) {
	...
	var validUrl = /^https?:\/\/((\d{1,3}\.){3}\d{1,3}|([-\w]+\.){2}(aero|arpa|asia|coop|info|jobs|mobi|museum|name|travel|[a-zA-Z]{2,3}))(:[0-9]+)?[-_/.\w]+$/;
...
//	if (!(this instanceof ManifestValidator)) { return new ManifestValidator; }
	this.errors    = [];
	this.resources = [];
...
ManifestValidator.prototype.checkResources = function(callback) {
	...
	function processRequest(res, lineNumber) {
		if ( [200, 301, 302, 307].indexOf(res.statusCode) === -1 ) {
			self.invalidResources[lineNumber] = url.resolve('http://' + res.connection._httpMessage.agent.host, res.connection._httpMessage.path);
		}
	...
	for (var i in this.resources) {
		if (this.resources.hasOwnProperty(i)) {
			...
			var resource = url.parse( url.resolve(this.baseUrl, this.resources[i]) ),
				options = {
			...
			(function(lineNumber){
				http.request(options, function(res){ processRequest(res, lineNumber); }).end();
			})(i);
		}
	}
};
