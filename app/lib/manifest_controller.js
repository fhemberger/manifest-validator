...
// e.g. ?foo=bar&foo=baz which Express turns into foo=['bar', 'baz']
function queryParameterToString(param) {
  return (param.constructor.name === 'Array')
    ? queryParameterToString(param[0])
    : param;
}
...
exports.dispatchPOST = function(req, res, dispatchFunction) {
  ...
  var parseFile = function(fieldname, file, filename, encoding, mimetype) {
    var fileContents = '';
  ...
  var parseField = function(fieldname, val) {
  ...
  try {
    ...
    // Catch Busboy errors, e.g. when POSTing JSON encoded data
    if (config.logging.application.enabled) { console.error(e); }
    callback(ERR_INVALID_API_CALL);
  }
};
...
exports.dispatchGET = function(req, res, dispatchFunction) {
  ...
  if (req.query && req.query.uri) {
    return validateUri(queryParameterToString(req.query.uri), callback);
  }

  // Input fields: Direct Input
  if (req.query && req.query.directinput) {
    return validateDirectInput(queryParameterToString(req.query.directinput), callback);
  }
